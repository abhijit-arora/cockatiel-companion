...
*   **5.1. The Two-Tabbed Hub:** The main screen of the module will be organized into two distinct feeds that the user can switch between.
    ...
    *   **5.1.2. The Flock Feed ("Flutters"):** This section is a visual, continuous-scrolling feed for social sharing.
        ...
        *   **Interactions:** Users can "Like" (with a heart icon) and post text-based comments on any item in the feed. Comments are displayed in a dedicated detail screen in chronological order (oldest first) to ensure a readable conversational flow. Each comment will also have its own like system.
